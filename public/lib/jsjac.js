...
b64arrays();if(typeof(atob)=='undefined'){b64decode=function(s){return utf8d2t(b64t2d(s));}}else{b64decode=function(s){return btoa(unescape(encodeURIComponent(s)));}}
if(typeof(btoa)=='undefined'){b64encode=function(s){return b64d2t(utf8t2d(s));}}else{b64encode=function(s){return decodeURIComponent(escape(atob(s)));}}
...
JSJaCConnection.prototype.connect=function(oArg){this._setStatus('connecting');if(oArg.authtype!='x-facebook-platform'){this.domain=oArg.domain||'localhost';this.username=oArg.username;this.resource=oArg.resource;this.pass=oArg.pass;this.register=oArg.register;}else{this.domain='chat.facebook.com';this.authtype=oArg.authtype;if(oArg.facebookApp!=undefined){this._facebookApp=oArg.facebookApp;if(!document.getElementById('fb-root')){fbDiv=document.createElement('div');fbDiv.id='fb-root';document.body.appendChild(fbDiv);}
...
this._xmllang='en';this.host=oArg.host;this.port=oArg.port||5222;this.authhost=this.host||this.domain;if(oArg.secure)
...
if(!this.connected())
return false;if(this._xmllang&&!packet.getXMLLang())
packet.setXMLLang(this._xmllang);if(cb){if(!packet.getID())
...
JSJaCConnection.prototype._doStreamBind=function(){var iq=new JSJaCIQ();iq.setIQ(null,'set','bind_1');iq.appendNode("bind",{xmlns:"urn:ietf:params:xml:ns:xmpp-bind"},[["resource",this.resource]]);this.oDbg.log(iq.xml());this.send(iq,this._doXMPPSess);};JSJaCConnection.prototype._doXMPPSess=function(iq){if(iq.getType()!='result'||iq.getType()=='error'){this.disconnect();if(iq.getType()=='error')
...
this._connected=false;var condition=body.getAttribute('condition');if(condition=="remote-stream-error")
...
return null;};var JSJaC={Version:'$Rev$',require:function(libraryName){document.write('<script type="text/javascript" src="'+libraryName+'"></script>');},load:function(){var includes=['xmlextras','jsextras','crypt','JSJaCConfig','JSJaCConstants','JSJaCCookie','JSJaCJSON','JSJaCJID','JSJaCBuilder','JSJaCPacket','JSJaCError','JSJaCKeys','JSJaCConnection','JSJaCHttpPollingConnection','JSJaCHttpBindingConnection','JSJaCConsoleLogger','JsJaCFBApplication'];var scripts=document.getElementsByTagName("script");var path='./';for(var i=0;i<scripts.length;i++){if(scripts.item(i).src&&scripts.item(i).src.match(/JSJaC\.js$/)){path=scripts.item(i).src.replace(/JSJaC.js$/,'');break;}}
